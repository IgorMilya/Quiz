...
const Checkbox: FC<CheckboxProps> = ({ answer }) => {
  const [isChecked, setIsChecked] = useState(false)
  const isCheckedBgClass = isChecked ? 'bg-primary-pinkDark' : 'bg-[#6D4376]'
  const labelClass = isChecked ? 'bg-primary-pinkDark bg-opacity-20 border-2 border-primary-pinkDark ' : 'bg-secondary'
  ...
  return (
    <label
      className={`cursor-pointer rounded-[16px] text-white flex items-center justify-between w-full pt-[17px] pb-[17px] pl-[20px] pr-[20px] ${labelClass}`}>
      ...
      />
      <div
        className={`w-6 h-6 flex items-center justify-center border border-primary-pinkDark rounded-[8px] transition-colors duration-300 ${isCheckedBgClass}`}>
        {isChecked && (
